-- ============================================================
-- True Deal — Schema inicial
-- Rodar no Supabase SQL Editor (app.supabase.com > SQL Editor)
-- ============================================================

-- Extensões
create extension if not exists "uuid-ossp";
create extension if not exists "pg_trgm"; -- busca por nome/username

-- ── Enums ────────────────────────────────────────────────────────────────────

create type deal_type         as enum ('oficial', 'privado', 'publico');
create type deal_mode         as enum ('regular', 'super');
create type deal_status       as enum ('formacao', 'ativo', 'finalizado');
create type deal_category     as enum ('social', 'fitness', 'checkin', 'free');
create type distribution_type as enum ('winner', 'top3', 'proportional');
create type payment_method    as enum ('pix', 'cripto', 'cartao');
...
-- Índice de busca por username (Hall of Fame)
...
create table public.deals (
  ...
  verification_type     text            not null,  -- 'social_followers', 'fitness_steps', etc.
...
create index deals_creator_id    on public.deals(creator_id);
create index deals_status        on public.deals(status);
create index deals_type          on public.deals(type);
create index deals_start_date    on public.deals(start_date);
...
create table public.deal_participants (
  ...
  start_snapshot   jsonb,   -- { "value": 12840, "metric": "followers" }
  current_snapshot jsonb,
  rank             integer,
  unique(deal_id, user_id)
);

create index deal_participants_deal_id on public.deal_participants(deal_id);
create index deal_participants_user_id on public.deal_participants(user_id);
...
-- ── TDP Transactions (ledger imutável) ───────────────────────────────────────

create table public.tdp_transactions (
  id         uuid        primary key default uuid_generate_v4(),
  user_id    uuid        not null references public.profiles(id) on delete cascade,
...
create index tdp_transactions_user_id on public.tdp_transactions(user_id);

-- Atualiza tdp_points no profile automaticamente
...
-- Atualiza streak e concede TDP no checkin
...
-- Profiles: qualquer um pode ver, só o dono edita
create policy "profiles_select" on public.profiles for select using (true);
create policy "profiles_update" on public.profiles for update using (auth.uid() = id);

-- Deals: públicos e oficiais visíveis para todos; privados só para participantes/criador
...
-- Participantes: visíveis para os participantes do deal
...
-- Invites: só quem recebeu ou enviou
...
-- TDP: só o dono vê suas transações
